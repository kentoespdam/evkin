				...
				>
					<div className="mt-3 space-y-3">
						{errors?.length ? (
							...
							<>
								{/** biome-ignore lint/a11y/useSemanticElements: using role="list" for semantic structure */}
								<div
									ref={codesSectionRef}
								...
								>
									{recoveryCodesList.length ? (
										recoveryCodesList.map((code) => (
											// biome-ignore lint/a11y/useSemanticElements: div used with role="listitem" for semantic list structure
											<div key={code} role="listitem" className="select-text">
												{code}
											</div>
										))
									) : (
										// biome-ignore lint/a11y/useAriaPropsSupportedByRole: div with aria-label for loading state context
										<div className="space-y-2" aria-label="Loading recovery codes">
											{Array.from({ length: 8 }, (_, index) => (
												<div
													// biome-ignore lint/suspicious/noArrayIndexKey: skeleton loading state with stable list length
													key={index}
