import { ArrowLeftIcon, ArrowRightIcon, ChevronsLeftIcon, ChevronsRightIcon, type LucideIcon } from "lucide-react";
import { memo } from "react";
import { usePaginationHandler } from "@/hooks/use-pagination";
import type { Pagination, PaginationMeta, PaginationMetaLink } from "@/types";
...
const NavButton = memo(({ type, meta, handleSelectChange }: NavButtonProps) => {
	...
	return (
		<ButtonGroup>
			<Button variant="outline" size="icon" onClick={() => handleSelectChange({ page: targetPage.toString() })}>
...
const PageList = memo(({ items, selected, handleSelectChange }: PageListProps) => (
	<div className="flex items-center gap-2">
		<Select value={selected?.page?.toString() ?? "1"} onValueChange={(value) => handleSelectChange({ page: value })}>
...
const PaginationNav = ({ page }: PaginationNavProps) => {
	const { meta, handleSelectChange, items, selected } = usePaginationHandler(page);

	return (
		<div className="flex justify-end px-4 items-center gap-2">
			<PageSize meta={meta} handleSelectChange={handleSelectChange} />
			<TotalPage meta={meta} />
			<NavButton type="first" meta={meta} handleSelectChange={handleSelectChange} />
			<NavButton type="prev" meta={meta} handleSelectChange={handleSelectChange} />
			<PageList items={items} selected={selected} handleSelectChange={handleSelectChange} />
			<NavButton type="next" meta={meta} handleSelectChange={handleSelectChange} />
			<NavButton type="last" meta={meta} handleSelectChange={handleSelectChange} />
		</div>
	);
};
