...
const PerhitunganReportsTableBody = memo(
	({ groupedReports, reportsMap, months, year }: PerhitunganReportsTableBodyProps & { year: number }) => {
		...
		const renderMonthCells = useCallback(
			(item: PerhitunganReportProps["masterReports"][number]) => {
				const withDesember = Array.from({ length: 13 }).map((_, i) => ({
					month: i === 12 ? 12 : i + 1,
